...
import { getEvmBech32Address, privateToHexAddress, privateToHexPublic } from 'app/lib/eth-helpers'
...
export function* openWalletFromEthereumPrivateKey({ payload: privateKey }: PayloadAction<string>) {
  try {
    const publicKey = privateToHexPublic(privateKey)
    const address = privateToHexAddress(privateKey)
    const oasisAddress = yield* call(getEvmBech32Address, address)
    const balance = yield* call(getBalance, hex2uint(publicKey))
    ...
    yield* put(
      walletActions.addWallet({
        ...
        publicKey,
        privateKey,
        type: WalletType.EthereumPrivateKey,
        balance,
        selectImmediately: true,
      }),
    )
  } catch (error: any) {
    throw new WalletError(WalletErrors.EthereumPrivateKeyUnknownError, error)
  }
}
