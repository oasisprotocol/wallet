...
export const accountSlice = createSlice({
  ...
  reducers: {
    ...
    transactionsLoaded(state, action: PayloadAction<TransactionsLoadedPayload>) {
      const {
        payload: { networkType, transactions, total },
      } = action

      state.transactionsError = undefined
      state.transactions = transactions
      state.totalNumOfTxs = total

      state.pendingTransactions[networkType] = state.pendingTransactions[networkType].filter(
    ...
    addPendingTransaction(state, action: PayloadAction<PendingTransactionPayload>) {
      ...
      state.pendingTransactions[networkType] = [transaction, ...state.pendingTransactions[networkType]]
    },
